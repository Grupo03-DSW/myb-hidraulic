    ...
    return proyectos.map((proyecto) => {
      return {
        ...
        repuestos: repuestos.filter(
          (repuesto) =>
            repuesto.idRepuesto !== undefined &&
            proyecto.infoRepuestos.ids_repuesto.includes(repuesto.idRepuesto)
        ),
      } as Proyecto;
    });
  } catch (err) {
...
export async function obtenerRepuestosPorProyecto(
  idProyecto: number
): Promise<Repuesto[]> {
  try {
    const res = await pool.query(
      "SELECT * FROM paObtenerRepuestosPorProyecto($1)",
      [idProyecto]
    );

    const repuestos: Repuesto[] = res.rows.map(
      (repuesto: {
        id_repuesto: number;
        nombre: string;
        descripcion: string;
...
export async function cambiarEtapaProyecto(
  idProyecto: number,
  idEtapa: number,
  fechaInicio: Date
) {
  try {
    await pool.query("SELECT paCambiarEtapaProyecto($1, $2, $3)", [
      idProyecto,
      idEtapa,
      fechaInicio,
    ]);
...
export async function obtenerEmpleadosPorIds(
  idsEmpleados: number[]
): Promise<Empleado[]> {
  try {
    const res = await pool.query("SELECT * FROM paObtenerEmpleadosPorIds($1)", [
      idsEmpleados,
    ]);
...
export async function obtenerClientesPorIds(
  idsClientes: number[]
): Promise<Cliente[]> {
  try {
    const res = await pool.query("SELECT * FROM paObtenerClientesPorIds($1)", [
      idsClientes,
    ]);
...
export async function obtenerEtapaPorId(idEtapa: number): Promise<string> {
  try {
    const res = await pool.query("SELECT * FROM paObtenerEtapaPorId($1)", [
      idEtapa,
    ]);
...
export async function obtenerProyectosPorJefe(
  idJefe: number
): Promise<Proyecto[]> {
  try {
    const res = await pool.query(
      "SELECT * FROM paxobtenerproyectoporjefe($1)",
      [idJefe]
    );
...
export async function obtenerProyectosPorSupervisor(
  idSupervisor: number
): Promise<Proyecto[]> {
  try {
    const res = await pool.query(
      "SELECT * FROM paxobtenerproyectoporsupervisor($1)",
      [idSupervisor]
    );
...
export async function obtenerProyectoPorTecnico(
  idTecnico: number
): Promise<Proyecto> {
  try {
    console.log("idTecnico", idTecnico);
    const res = await pool.query(
      "SELECT * FROM paobtenerproyectoportecnico($1)",
      [idTecnico]
    );
    ...
    console.log(res.rows[0].paobtenerproyectoportecnico);
...
export async function obtenerProyectoPorId(
  ...
  try {
    const res = await pool.query("SELECT * FROM paObtenerProyectosPorId($1)", [
      idProyecto,
    ]);
...
export async function asignarEmpleadosAProyecto(data: {
  ...
  try {
    ...
    await pool.query("SELECT paAsignarEmpleadosAProyecto($1, $2, $3)", [
      idProyecto,
      idEmpleados,
      fechaAsignacion,
    ]);
...
export async function obtenerHistorialProyecto(
  idProyecto: number
): Promise<HistorialProyecto> {
  try {
    // Llama al procedimiento almacenado con el ID del proyecto
    const res = await pool.query(
      "SELECT paObtenerHistorialProyecto($1) AS historial",
      [idProyecto]
    );
