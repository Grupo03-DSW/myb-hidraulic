...
export async function registrarRepuesto(repuesto: Repuesto) {
  try {
    await pool.query("CALL paRegistrarRepuesto($1, $2, $3, $4, $5)", [
      repuesto.nombre,
      repuesto.precio,
      repuesto.descripcion,
      repuesto.linkImg || null,
...
export async function actualizarStockRepuesto(
  cantidadObtenida: number,
  idRepuesto: number
) {
  try {
    await pool.query("CALL paActualizarStock($1, $2)", [
      idRepuesto,
      cantidadObtenida,
    ]);
  } catch (err) {
...
export async function actualizarStockRepuestos(
  repuestos: { idRepuesto: number; cantidadObtenida: number }[]
) {
  try {
...
export async function obtenerRepuestosRequeridos() {
  try {
    const res = await pool.query(
      "SELECT * FROM paobtenerrepuestosrequeridos()"
    );
    const repuestos: Repuesto[] = res.rows.map(
      (repuesto: {
        id_repuesto: number;
...
export async function agregarRepuestosSolicitados(
  repuestos: { idRepuesto: number; cantidadSolicitada: number }[]
) {
  try {
    ...
    await pool.query("CALL paAgregarRepuestosSolicitados($1, $2)", [
      idsRepuestos,
      cantidades,
    ]);
  } catch (err) {
...
export async function obtenerRepuestosPorIds(
  idsRepuestos: number[]
): Promise<Repuesto[]> {
  try {
    const res = await pool.query("SELECT * FROM paObtenerRepuestosPorIds($1)", [
      idsRepuestos,
    ]);
    const repuestos: Repuesto[] = res.rows.map(
      (repuesto: {
...
export async function obtenerRepuestosFaltantesPorJefe(jefeId: number) {
  try {
    const res = await pool.query(
      "SELECT * FROM paObtenerRepuestosFaltantes($1)",
      [jefeId]
    );
    const repuestosFaltantes = res.rows.map(
      (repuesto: { ids_repuestos: number[]; cantidades: number[] }) => {
        return repuesto.ids_repuestos === null
          ? []
          : repuesto.ids_repuestos.map((idRepuesto, index) => {
              return {
    ...
    const repuestos = await obtenerRepuestosPorIds(
      repuestosFaltantes.map((repuesto) => repuesto.idRepuesto)
    );
    ...
    if (err instanceof Error) {
      console.error(
        "Error al obtener repuestos faltantes por jefe:",
        err.stack
      );
    } else {
